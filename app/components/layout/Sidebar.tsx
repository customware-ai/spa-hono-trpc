...
export function Sidebar(): ReactElement {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 h-screen bg-surface-900 border-r border-surface-800 transition-all duration-300 ease-out z-40 ${
        collapsed ? "w-16" : "w-64"
      }`}
    >
      <div className="flex flex-col h-full">
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-surface-800">
          ...
          >
            <svg
              className={`w-5 h-5 text-surface-400 transition-transform ${collapsed ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
